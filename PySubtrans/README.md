...
`use_netflix_timing_guide`: Extends short subtitles following Netflix's timing guide instead of `seconds_per_character`. The reading time allows for the script: at Netflix's adult reading speeds, alphabetic text is read at 20 characters per second, Chinese at 9, Korean at 12 and Japanese at 4. Spaces count in alphabetic text but not in East Asian scripts, and combining marks such as Thai vowel and tone marks are not counted. Subtitles are extended to at least `min_line_duration`, up to 2 frames before the next subtitle, and gaps of 3 to 11 frames (at 24 fps) are closed to 2 frames. `netflix_timings_adjustment` scales the reading speed as a percentage (default 100): higher values give shorter subtitles, and 0 applies only the minimum duration and gap rules.
