...
- `python tests/integration_tests.py`: all concrete-provider tests, provider registration/import checks, and real FFmpeg/ffprobe chunking. API responses are mocked; no live API requests or model-weight loading are performed.
...
Integration tests report skips when optional SDKs or FFmpeg/ffprobe are absent. Install the relevant dependencies to exercise those checks; a successful run with skips does not validate those integrations. SDK import errors when installed fail the suite.
