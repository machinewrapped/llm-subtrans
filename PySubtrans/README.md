...
`use_netflix_timing_guide`: Extends short subtitles following Netflix's [Timed Text Style Guides](https://partnerhelp.netflixstudios.com/hc/en-us/sections/22463232153235-Timed-Text-Style-Guides) instead of `seconds_per_character`. Reading time uses Netflix's adult reading speed limit for the target language: 17 characters per second for most languages, 20 for English and Arabic, 22 for Hindi and other Indic languages, 12 for Korean, 9 for Chinese and 7 for Japanese. Where Netflix gives a separate limit for SDH, as for Japanese, that limit is used, since LLM translations are closer to verbatim than professionally condensed subtitles. When the target language is not recognised, it is detected from the script of each subtitle. Characters are counted as Netflix counts them: spaces and punctuation count, half-width characters count as half in Chinese, Japanese and Korean, and Thai tone marks and upper and lower vowels are not counted. Subtitles are extended to at least `min_line_duration` and at most 7 seconds, up to 2 frames before the next subtitle, and gaps of 3 to 11 frames (at 24 fps) are closed to 2 frames. `netflix_timings_adjustment` scales the reading speed as a percentage (default 100): higher values give shorter subtitles, and 0 applies only the minimum duration and gap rules.
